...
They all appear to have the same owner type "licensed." 
...
#is this right????
  Without changing the latitude or longitude normally a bigger number would zoom in more however in this case it does not. 
...
ggmap(twin_cities_starbucks) + # creates the map "background"
  ...
  theme_map() +
  theme(legend.background = element_blank()) + 
  geom_point(aes(x =-93.170278, y = 44.9382502, color = "blue")) +
...
census_pop_est_2018 <- read_csv("https://www.dropbox.com/s/6txwv3b4ng7pepe/us_census_2018_state_pop_est.csv?dl=1") %>% #Renames this whole code chunk to be located with the name census_pop_est_2018. It also reads in the csv file with the data from census. 
  separate(state, into = c("dot","state"), extra = "merge") %>% #Separates the dot and the state name, the extra part of the function makes sure that states with two words don't get seperated into seperate rows (ex. north dakota)
...
```{r}
total_departures_withlatlon <- Stations %>%
  left_join(Trips,
            by = c("name"= "sstation")) %>%  
  group_by(name) %>% 
  mutate(total_departures =  n()) 

total_departures_withlatlon

dep <- colorFactor("viridis", 
                     domain = total_departures_withlatlon$total_departures) 
...
#leaflet
#leaflet(data = total_departures_withlatlon) %>% 
  #addTiles() %>% 
  #addCircles(data = total_departures_withlatlon, 
             #lng = ~long, 
             #lat = ~lat, 
             #label = ~name, 
             #color = ~pal(name)) %>%
...
The biggest percent departures by causal users are in the tourist hot spots of DC. Near the mall and monuments. 
...
The Problem with this map is that it does not take into account the population of each state. So California, Texas, New York, and Florida look like they have a higher cumulative case count than other states, which is true. However without taking into account the population of each state it does not accuratley depict the rate of COVID-19 in each state. 
...
mpls_suspicious <- MplsStops %>% 
  ...
  summarize(total_stops = n(), 
            prop_susp = sum(susp)/total_stops) %>% 
...
```{r}
mplsDemStop <- mpls_suspicious %>% 
  left_join(MplsDemo, by = "neighborhood")

mpls_all <- mpls_nbhd %>% 
  left_join(mplsDemStop, by = c("BDNAME" = "neighborhood")) %>% 
